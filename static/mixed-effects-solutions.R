...
source("mixed-effects-simulations.R")
source("mixed-effects-plots.R")
...
# Suppose the expected female blood pressure (BP) is 112
# and that the expected male BP is 128.
...
set.seed(1)
S <- simulateRM(c(mu.f, mu.m), c(n.f, n.m), sigma.s, n.rep, sigma)
x <- S$x
...
attr(summary(m)$varcor$subject, "stddev") # sigma.s hat
sigma.s # compare to true sigma.s used in simulation
...
m <- lmer(sat ~ sex + (1 | school/student), data=x)
m

# Note: Because the subjects are given unique ids
# rather than ids being repeated within different schools,
# it would be equivalent to use the formula
# lmer(sat ~ sex + (1 | student) + (1 | school), data=x)
...
ci <- confint(m)
ci[3, ]
# the true value isn't inside

# use the "help" page to see how to specify a different confidence level
confint(m, level=.99)[3, ]
# it still doesn't quite cover the true value

# Test null hypothesis that all five experiments have the same expectation.
ranova(m)
# The significance probability is .01,
# indicating that we should reject the null.
# Additionally, the simpler model has a higher AIC,
# which also points to its inferiority.
...
# The scenario is similar to our previous longitudinal growth example.
# However, in this case, the intercept isn't zero
# (the chickens have non-zero weights at their birth).
# Notice that we shouldn't allow the different diets to have different
# intercepts since the chickens haven't yet had any food at time 0 (birth).
# Each chicken should be given its own random intercept effect
# and random slope effect.
# The errors have autocorrelation as before.

m <- lme(sqrtweight ~ Diet:Time, random=~Time|Chick, data=x, correlation=corAR1(), control=ctrl)
# optimization error!

# found a work-around at https://stats.stackexchange.com/questions/40647/lme-error-iteration-limit-reached
ctrl <- lmeControl(opt='optim')
m <- lme(sqrtweight ~ Diet:Time, random=~Time|Chick, data=x, correlation=corAR1(), control=ctrl)

fixef(m)
int.hat <- fixef(m)[1]
slopes.hat <- fixef(m)[-1]
k <- length(slopes.hat)

# Plot of data and estimated lines
plot(x$Time, sqrt(x$weight), type="n", xlab="day", ylab="square root of weight",
     main="Growth of chickens on various diets")
for(i in 1:n) {
  these <- which(format(x$Chick)==format(i))
  lines(x$Time[these], x$sqrtweight[these], col=adjustcolor(x$Diet[these[1]], alpha.f=.4))
}
for(j in 1:k) {
  abline(int.hat, slopes.hat[j], lty=2, lwd=6, col=j)
}
legend("topleft", legend=levels(x$Diet), fill=levels(x$Diet))
...
emmeans(m, pairwise ~ Diet)
# Diet 1 is significantly better than diets 3 and 4.
# Note the version of this function call that would
# match up with the coefficient estimates is
# emmeans(m, pairwise ~ Diet, at=list(Time=1), offset=-fixef(m)[1])
